...
SELECT 
    ...
    (SELECT COUNT(*) 
     ...
     WHERE s.plan_id = p.plan_id AND s.start_date >= '2021-01-01') AS event_2021
FROM plans AS p
ORDER BY p.plan_id;
...
SELECT 
    ...
    ROUND((CAST(total_customers.customer_count AS FLOAT) / total_subscribers.total_count) * 100, 1) AS churn_percentage
FROM 
    (SELECT COUNT(*) AS customer_count FROM subscriptions WHERE plan_id = 4) AS total_customers,
    (SELECT COUNT(DISTINCT customer_id) AS total_count FROM subscriptions) AS total_subscribers;
...
SELECT
    next_plan,
    COUNT(*) AS num_cust,
    ROUND(COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT customer_id) FROM subscriptions), 1) AS perc_next_plan
FROM (
    SELECT
        LEAD(plan_id, 1) OVER (PARTITION BY customer_id ORDER BY plan_id) AS next_plan
    FROM subscriptions
) AS next_plans
WHERE next_plan IS NOT NULL AND next_plan != 0
GROUP BY next_plan
ORDER BY next_plan;
...
) AS unique_customers_query;




SELECT
    ROUND(AVG(DATEDIFF(ap.start_date, tp.start_date)), 0) AS avg_upgrade
FROM
    subscriptions ap
JOIN
    subscriptions tp ON ap.customer_id = tp.customer_id
WHERE
    ap.plan_id = 3
    AND tp.plan_id = 0;
